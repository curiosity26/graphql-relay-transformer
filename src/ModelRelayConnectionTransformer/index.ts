...
import { CONDITIONS_MINIMUM_VERSION } from 'graphql-dynamodb-transformer';
import {
  getBaseType,
  isListType,
...
} from '../DynamoDBRelayModelTransformer/definitions';
...
export class ModelRelayConnectionTransformer extends Transformer {
  ...
  public before = (ctx: TransformerContext): void => {
    ...
    template.Resources && ctx.mergeResources(template.Resources);
    template.Parameters && ctx.mergeParameters(template.Parameters);
    template.Outputs && ctx.mergeOutputs(template.Outputs);
  };
  ...
  public field = (
    ...
    let associatedSortFieldName: string | null = null;
    let sortType = null;
    ...
    const associatedSortField = parent.fields?.find(
      (f: FieldDefinitionNode) => associatedSortFieldName && f.name.value === associatedSortFieldName,
    );
    ...
    const foreignAssociatedSortField = relatedType.fields?.find((f: FieldDefinitionNode) => f.name.value === associatedSortFieldName);
    ...
    if (leftConnectionIsList && rightConnectionIsList) {
      ...
      existingKeyField && validateKeyField(existingKeyField);
      ...
      const sortField = { name: associatedSortFieldName as unknown as string, type: sortType as unknown as string };
      const updated = this.resources.updateTableForConnection(table, connectionName, connectionAttributeName, sortField);
      updated && ctx.setResource(tableLogicalId, updated);
  ...
  public connectionWithKey = (
    ...
    args.fields.forEach(item => {
      ...
      inputFields.push(parent.fields?.find(f => f.name.value === item) as FieldDefinitionNode);
    ...
    let index: LocalSecondaryIndex | undefined | null;
