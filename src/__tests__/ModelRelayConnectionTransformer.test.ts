...
import { ModelRelayConnectionTransformer } from '../ModelRelayConnectionTransformer';
const featureFlags = {
...
test('Test ModelRelayConnectionTransformer simple one to many happy case', () => {
  ...
  expect((commentField?.type as any).name.value).toEqual('RelayModelCommentConnection');
...
test('Test that ModelRelayConnection Transformer throws error when the field in connection is not found in the related Type', () => {
  ...
  try {
    ...
    console.log(e);
    expect(e).toBeTruthy();
    expect((e as Error).name).toEqual('InvalidDirectiveError');
  }
});
...
test('Test ModelRelayConnectionTransformer many to many should fail due to missing other "name"', () => {
  ...
  const schemaDoc = parse(out.schema);
  const postType = getObjectType(schemaDoc, 'Post');
  const postConnection = getObjectType(schemaDoc, 'RelayModelPostConnection');
  const thingConnection = getObjectType(schemaDoc, 'RelayModelThingConnection');
  const thingEdge = getObjectType(schemaDoc, 'RelayModelThingEdge');
  const thingFilterInput = getInputType(schemaDoc, 'RelayModelThingFilterInput');
...
function expectArguments(field: FieldDefinitionNode | undefined, args: string[]) {
