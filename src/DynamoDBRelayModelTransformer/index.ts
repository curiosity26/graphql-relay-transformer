...
import { DirectiveNode, ObjectTypeDefinitionNode, InputObjectTypeDefinitionNode, FieldDefinitionNode, TypeNode } from 'graphql';
import {
  blankObject,
  makeField,
...
export class DynamoDBRelayModelTransformer extends Transformer {
  ...
  public before = (ctx: TransformerContext): void => {
    ...
    template.Conditions && ctx.mergeConditions(template.Conditions);
  };
  ...
  public after = (ctx: TransformerContext): void => {
    ...
    if (ddbMetata) {
      Object.entries(ddbMetata.hoistedRequestMappingContent || {}).forEach(([resourceId, hoistedContentGenerator]) => {
        const hoistedContent = (hoistedContentGenerator as Function)();
        if (hoistedContent) {
  ...
  public object = (def: ObjectTypeDefinitionNode, directive: DirectiveNode, ctx: TransformerContext): void => {
    ...
    if (isSyncEnabled) {
      const obj = ctx.getObject(def.name.value) as ObjectTypeDefinitionNode;
      const newFields = [
        ...(obj.fields ?? []),
  ...
  private addTimestampFields(def: ObjectTypeDefinitionNode, directive: DirectiveNode, ctx: TransformerContext): void {
    ...
    const existingCreatedAtField = def.fields?.find(f => f.name.value === createdAtField) as FieldDefinitionNode;
    const existingUpdatedAtField = def.fields?.find(f => f.name.value === updatedAtField) as FieldDefinitionNode;
    ...
    if (!DynamoDBRelayModelTransformer.isTimestampCompatibleField(existingCreatedAtField)) {
      console.log(
        `${def.name.value}.${existingCreatedAtField?.name.value} is of type ${getBaseType(
          existingCreatedAtField?.type as TypeNode,
  ...
  private createMutations = (
    ...
    if (shouldMakeCreate) {
      ...
      const createResolver = this.resources.makeCreateResolver({
        ...
        syncConfig: this.opts.SyncConfig as SyncConfig,
      });
