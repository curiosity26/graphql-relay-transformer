...
export function makeNonModelInputObject(
  ...
  const fields: InputValueDefinitionNode[] | undefined = obj.fields
...
export const modelEdgeTypeName = (typeName: string): string => {
  return `RelayModel${typeName}Edge`;
};
  ...
  if (sortKeyInfo) {
    ...
    if (sortKeyInfo.typeName === 'Composite') {
      namedType = makeNamedType(
        ModelResourceIDs.ModelCompositeKeyConditionInputTypeName(sortKeyInfo.model!, toUpper(sortKeyInfo.keyName!)),
      );
    } else {
